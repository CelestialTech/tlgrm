...
8. **Panel visibility (MCP)**: `setHideOnDeactivate(false)` is set in both `showProgress()` and `startExportNow()` so the export panel stays visible even when deactivated. Title is hardcoded to "Exporting" to avoid cloud lang pack overrides. `setQuitOnLastWindowClosed(false)` in `Sandbox` constructor prevents macOS/Qt from terminating the app when the export panel temporarily loses focus.

9. **Auto-resume on startup** (`tryAutoResumeExport` in `mcp_server_complete.cpp`): On session connect, the MCP server polls for `Core::App().activeWindow()` via a 200ms QTimer (max 150 tries / 30s). Once the window is ready, it scans for `resume_state.json` files in the export directory. The JSON stores serialized `MTPInputPeer` fields (`input_peer_type`, `input_peer_id`, `input_peer_hash`) so the correct peer identity is reconstructed — `peer->input()` is unreliable for unresolved peers. Falls back to TDF settings if no JSON found.

10. **Crash resilience in file downloads**: `loadNextMessageFile()` has a `_fileProcess` re-entrancy guard to prevent assertion crashes when gradual-mode pacing timers fire while a file download is already in progress. Files with expired references (`dcId=0`) are skipped in `processFileLoad()` with `SkipReason::Unavailable` instead of crashing. The `Ensures` assertion in `loadFile()` is replaced with graceful fallback that clears `_fileProcess` and calls `done(QString())`.

11. **Peer identity in auto-resume**: `startAutoExport()` prefers `settings.singlePeer` (with access hash from TDF/JSON) over `peer->input()`. Returns early if both are `inputPeerEmpty`. The `writeResumeStateJson()` in `export_controller.cpp` serializes the full `MTPInputPeer` (type/id/hash) so channel exports survive app restarts.

12. **Stale fragment cleanup**: `writeDialogStart()` in `export_output_html.cpp` calls `QDir(_fragmentsDir).removeRecursively()` before `QDir().mkpath(_fragmentsDir)` to clear stale fragments from previous interrupted exports.

**Key files**: `mcp_archive_tools.cpp` (resume detection, truncated file cleanup, Settings construction), `mcp_server_complete.cpp` (`tryAutoResumeExport`, MTPInputPeer reconstruction from JSON), `export_controller.cpp` (progress tracking, cancel state, `writeResumeStateJson`), `export_api_wrap.cpp` (message fetching, `currentResumeProgress()`, gradual mode, `_fileProcess` guard, dcId=0 skip), `export_view_panel_controller.cpp` (panel visibility, `setHideOnDeactivate(false)`), `export_manager.cpp` (`startAutoExport` with peer validation), `export_output_abstract.cpp` (directory reuse), `core/sandbox.cpp` (`setQuitOnLastWindowClosed(false)`), `storage_account.cpp` (settings serialization with `gradualMode`), `export_settings.h` (`singlePeerResumeFromId`, `singlePeerResumeSkipCount`, `resumeExportDir`).
