...
send_cdp() {
  ...
  printf '%s' "$msg" | websocat -B 10000000 -n1 "$ws_url" 2>/dev/null
}
...
for t in tabs:
  ...
  screenshot)
    OUTFILE="${1:-/tmp/viewport-screenshot.jpg}"
    TMPJSON="/tmp/cdp_screenshot_$$.json"
    send_cdp "Page.captureScreenshot" '{"format":"jpeg","quality":80}' > "$TMPJSON"
    python3 -c "
import json, base64
