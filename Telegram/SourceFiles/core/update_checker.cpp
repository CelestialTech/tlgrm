...
void MtpChecker::start() {
	...
	// Public channel carrying the signed update packages. Tlgrm ships a
	// single macOS updater generation, so this is a literal name rather
	// than upstream's base + AutoUpdateVersion() suffix (tdhbcfeed ->
	// tdhbcfeed4). Named to match the HTTP autoupdate host
	// updates.71grm.site so both delivery paths read as one system.
	const auto feed = u"updates71grm"_q;
