...
|---|---|---|
| **MCP** | The aggregated tool socket: a collapsible tool tree across **14 domains** (the live endpoint serves 404 tools); select a tool to see its description + params; fill params (scalars, and array/object via JSON) and **Invoke** read tools; a **live-traffic monitor** (recent calls + per-minute rate) and a **recent-invokes history**. Mutating tools are guarded — run from their own plugin. | `tools/list`, any `get_/list_/search_/count_` |
